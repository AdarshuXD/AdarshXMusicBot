...
from AdarshXMusic import YouTube, app
from AdarshXMusic.core.call import AdarshX
from AdarshXMusic.misc import SUDOERS, db
from AdarshXMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from AdarshXMusic.utils.decorators.language import languageCB
from AdarshXMusic.utils.formatters import seconds_to_min
from AdarshXMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from AdarshXMusic.utils.stream.autoclear import auto_clean
from AdarshXMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await AdarshX.pause_stream(chat_id)
        ...
        await AdarshX.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await AdarshX.stop_stream(chat_id)
        ...
        await AdarshX.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await AdarshX.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await AdarshX.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await AdarshX.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await AdarshX.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
